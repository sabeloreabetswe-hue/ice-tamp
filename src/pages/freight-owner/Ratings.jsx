...
import { getRatingsByFreightOwner } from "../../services/ratingService";
...
const Ratings = () => {
  ...
  const loadData = () => {
    ...
    // Shipments where this user is the freight owner that are delivered
    const delivered = allShipments.filter(
      (s) =>
        s.freightOwnerEmail === user.email &&
        s.shipmentStatus === "Delivered"
    );

    setDeliveredShipments(delivered);

    // All ratings given by this freight owner
    const ratings = getRatingsByFreightOwner(user.email);
